    ...
    public class Connection : IDisposable
    {
        ...
        public Connection(Bancos banco = Bancos.Sgq)
        {
            ...
            if (this.Banco == Bancos.Sgq)
                this.ConnectionString = ConfigurationManager.ConnectionStrings["SGQ"].ConnectionString;
            else
                this.ConnectionString = ConfigurationManager.ConnectionStrings["BITI"].ConnectionString;
        ...
        public string Get_String(string sql)
        {
            ...
            if (sqlDataReader.Read())
                Retorno = sqlDataReader[0].ToString();
        ...
        {
            var Retorno = "";
            var sqlCommand = new SqlCommand("select " + Campo + " from " + Tabela + " where Id=" + Id, connection);
            SqlDataReader sqlDataReader = sqlCommand.ExecuteReader();
        ...
        {
            var sqlCommand = new SqlCommand(sql, this.connection);
        ...
        public int Setfields_Por_Id(string Tabela, string Id, string Campos, string Valores)
        {
